		...
		if ($io->fileExists($path)){
			...
			while (($row = $io->streamReadCsv()) !== false){
				$row = array_combine($header, $row);
					$sProduct = Mage::getModel('catalog/product');
					//$current_product_already_exists = $sProduct->loadByAttribute('sku',);
					$curr_pro_sku = (!empty($row['Subproduct Code']) ) ?  $row['Subproduct Code'] :  $row['Product Code'];
				...
				if($row['Subproduct Code'] !="")
				{
						...
						/*foreach ($simpleProducts as $simpleArray)
						{
							...
							foreach ($attributes_array as $key => $attrArray)
							{
								...
								$associatedProductData = array();
								foreach($simpleArray['attrData'] as $simpleData){
									
									array_push($associatedProductData, array(
												"attribute_id" => $simpleData['attr_id'],
						...
						foreach ($simpleProducts as $simpleArray) { $dataArray[$simpleArray['id']] = array(); foreach ($attributes_array as $attrArray) { array_push( $dataArray[$simpleArray['id']], array( "attribute_id" => $simpleArray['attr_id'], "label" => $simpleArray['label'], "is_percent" => false, "pricing_value" => $simpleArray['price'] ) ); } }  
