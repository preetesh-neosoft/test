...
class Neo_Neoimport_IndexController extends Mage_Core_Controller_Front_Action{
    ...
    public function fetchSubCategoryId($sub_category_parent)
	{
	...
	{
		$path = Mage::getBaseDir('media'). '/' ."magento_product_import.csv";
		...
		if ($io->fileExists($path)){
			...
			while (($row = $io->streamReadCsv()) !== false){
				...
				if($row['Subproduct Code']==""){
					...
					if(isset($row['Range'] != '' || $row['Range'] != NULL){
						array_push($categories,$this->fetchSubCategoryId($main_category_id));
					}
