			...
			while (($row = $io1->streamReadCsv()) !== false)
			{
				...
				die;*/
						...
						if($product_id1){
							...
							$cProduct = $data1;*/
							/*	$parent 	= Mage::getModel('catalog/product')->load($product_id1);
				            ...
				            $parentId = $product_id1;

				            $product = Mage::getResourceModel( 'catalog/product_type_configurable' )->load( $parentId );
				            $product->saveProducts( $parentId, array_values($childIds));*/
							...
							$color_attrib_options = explode('-',$row['Product Name']);
							$curr_prod_name = (isset($color_attrib_options[1])) ? $color_attrib_options[1] : $row['Product Name'];
							$productData=array(
									'name' => $curr_prod_name,
							...
							/*$simpleProducts1 = $simpleProductsArray[$row['Product Code']];
							foreach ($simpleProducts1 as $simpleArray) { 
								$dataArray[$simpleArray['id']] = array(); 
