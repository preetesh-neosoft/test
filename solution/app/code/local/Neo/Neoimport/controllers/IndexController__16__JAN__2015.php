		...
		if ($io->fileExists($path)){
			...
			while (($row = $io->streamReadCsv()) !== false){
				$row = array_combine($header, $row);
					$sProduct = Mage::getModel('catalog/product');
					//$current_product_already_exists = $sProduct->loadByAttribute('sku',);
					$curr_pro_sku_1 = (!empty($row['Subproduct Code']) ) ?  $row['Subproduct Code'] :  $row['Product Code'];
					...
					if($product_id){
						...
						if(isset($row['size_imp']) && $row['size_imp']!=""){
							
							$optionId = $this->addAttributeValue('size_imp',$row['size_imp']);
							//exit($optionId);
							...
							/*array_push(						
								...
								array(
									"attr_code" => 'size_imp',
									"attr_id" => $attrCodeData['size_imp'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['size_imp'],
								)
							);*/
						}
						if(isset($row['size_met']) && $row['size_met']!=""){
							
							$optionId = $this->addAttributeValue('size_met',$row['size_met']);
							//exit($optionId);
							...
							/*array_push(						
								...
								array(
									"attr_code" => 'size_met',
									"attr_id" => $attrCodeData['size_met'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['size_met'],
								)
							);*/
						}
						if(isset($row['rrp']) && $row['rrp']!=""){
							
							$optionId = $this->addAttributeValue('rrp',$row['rrp']);
							//exit($optionId);
							
							$sProduct->setData('rrp', $optionId);
							/*array_push(						
								$optionData,
								array(
									"attr_code" => 'rrp',
									"attr_id" => $attrCodeData['rrp'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['rrp'],
								)
							);*/
						...
						if(isset($row['mpn']) && $row['mpn']!=""){
							...
							/*array_push(						
								...
								array(
									"attr_code" => 'mpn',
									"attr_id" => $attrCodeData['mpn'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['mpn'],
								)
							);*/
						...
						if(isset($row['manufacturer']) && $row['manufacturer']!=""){
							...
							/*array_push(						
								...
								array(
									...
									"attr_id" => $attrCodeData['manufacturer'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['manufacturer'],
								)
							);*/
						}
						if(isset($row['is_man']) && $row['is_man']!=""){
							
							$optionId = $this->addAttributeValue('is_man',$row['is_man']);
							//exit($optionId);
							
							$sProduct->setData('is_man', $optionId);
							/*array_push(						
								$optionData,
								array(
									"attr_code" => 'is_man',
									"attr_id" => $attrCodeData['is_man'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['is_man'],
								)
							);*/
						...
						if(isset($row['is_woman']) && $row['is_woman']!=""){
							
							$optionId = $this->addAttributeValue('is_woman',$row['is_woman']);
							//exit($optionId);
							
							$sProduct->setData('is_woman', $optionId);
							/*array_push(						
								$optionData,
								array(
									"attr_code" => 'is_woman',
									"attr_id" => $attrCodeData['is_woman'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['is_woman'],
								)
							);*/
						}
						if(isset($row['is_lingerie']) && $row['is_lingerie']!=""){
							
							$optionId = $this->addAttributeValue('is_lingerie',$row['is_lingerie']);
							//exit($optionId);
							
							$sProduct->setData('is_lingerie', $optionId);
							/*array_push(						
								$optionData,
								array(
									"attr_code" => 'is_lingerie',
									"attr_id" => $attrCodeData['is_lingerie'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['is_lingerie'],
								)
							);*/
						}
						if(isset($row['is_couples']) && $row['is_couples']!=""){
							
							$optionId = $this->addAttributeValue('is_couples',$row['is_couples']);
							//exit($optionId);
							
							$sProduct->setData('is_couples', $optionId);
							/*array_push(						
								$optionData,
								array(
									"attr_code" => 'is_couples',
									"attr_id" => $attrCodeData['is_couples'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['is_couples'],
								)
							);*/
						}
						if(isset($row['is_woman']) && $row['is_woman']!=""){
						...
						if(isset($row['is_fetish']) && $row['is_fetish']!=""){
							
							$optionId = $this->addAttributeValue('is_fetish',$row['is_fetish']);
							//exit($optionId);
							
							$sProduct->setData('is_fetish', $optionId);
							/*array_push(						
								$optionData,
								array(
									"attr_code" => 'is_fetish',
									"attr_id" => $attrCodeData['is_fetish'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['is_fetish'],
								)
							);*/
						}
						if(isset($row['is_gay_male']) && $row['is_gay_male']!=""){
							
							$optionId = $this->addAttributeValue('is_gay_male',$row['is_gay_male']);
							//exit($optionId);
							
							$sProduct->setData('is_gay_male', $optionId);
							/*array_push(						
								$optionData,
								array(
									"attr_code" => 'is_gay_male',
									"attr_id" => $attrCodeData['is_gay_male'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['is_gay_male'],
								)
							);*/
						}
						if(isset($row['is_gay_female']) && $row['is_gay_female']!=""){
							
							$optionId = $this->addAttributeValue('is_gay_female',$row['is_gay_female']);
							//exit($optionId);
							
							$sProduct->setData('is_gay_female', $optionId);
							/*array_push(						
								$optionData,
								array(
									"attr_code" => 'is_gay_female',
									"attr_id" => $attrCodeData['is_gay_female'], // i have used the hardcoded attribute id of attribute size, you must change according to your store
									"value" => $optionId,
									"label" => $row['is_gay_female'],
								)
							);*/
						}
				...
				if($row['Subproduct Code'] !="")
				{
						...
						if($product_id1){
						    $current_product_already_exists1 = $cProduct->load($product_id1);
						    foreach($current_product_already_exists1 as $data1)
							$cProduct = $data1;
						}
						$productData=array(
				 	...
				 	/*print_r($attributes_array);die;*/
						...
						foreach ($simpleProducts as $simpleArray) { 
							...
							foreach ($attributes_array as $attrArray) { 
								array_push( $dataArray[$simpleArray['id']], array( "attribute_id" => $simpleArray['attr_id'], "label" => $simpleArray['label'], "is_percent" => false, "pricing_value" => $simpleArray['price'] ) ); 
							} 
						}

				$childIds = $cProduct->getTypeInstance()->getUsedProductIds();
                ...
                ->addAttributeToFilter('sku', array('like' => $row['Product Code'] . "__%"));
						...
						$cProduct->save();
						//echo "sdsadsadsad111111111111";die;
						$simpleProducts = array();
						$urlCounter      = 1;
